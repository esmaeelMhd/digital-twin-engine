...
def _packed_to_training_params(params: Float[Array, "11"]) -> Float[Array, "6"]:
    """Map packed simulator parameters to the historic stored 6D training vector."""
    return jnp.array(
        [
            params[0],
            params[2] / 1e10,
...
class UnifiedCSTRDataGenerator(GenericDataGenerator):
    """CSTR generator that reuses the generic pipeline with CSTR-specific signals."""

    def __init__(self, simulator: CSTRSimulator, config: dict):
        super().__init__(simulator, config, simulator.spec)
    ...
    def _generate_control_trajectory(
        ...
        F_in = self._generate_signal(
            key1, n_steps, dt, F_min, F_max, "mixed", switch_prob=0.05, n_changes=5
        )
        Tc_in = self._generate_signal(
            key2, n_steps, dt, Tc_min, Tc_max, "mixed", switch_prob=0.05, n_changes=5
        )
    ...
    def _generate_control_trajectories(
        ...
        n_steps: int,
        dt: float,
    ) -> Float[Array, "batch n_steps 2"]:
        key1, key2 = jax.vmap(jax.random.split)(keys).transpose(1, 0, 2)
        F_min, F_max = self.operating_ranges["F_in"]
        Tc_min, Tc_max = self.operating_ranges["Tc_in"]
        ...
        return jnp.concatenate([F_in, Tc_in], axis=2)
    ...
    def _generate_disturbance_trajectory(
        ...
        Ca_in = self._generate_signal(key1, n_steps, dt, Ca_min, Ca_max, "prbs", switch_prob=0.01)
        T_in = self._generate_signal(key2, n_steps, dt, T_min, T_max, "prbs", switch_prob=0.01)
    ...
    def _generate_disturbance_trajectories(
        ...
        return jnp.concatenate([Ca_in, T_in], axis=2)
    ...
    def _generate_trajectory(
        ...
        traj = super()._generate_trajectory(key, n_steps, dt, simulation_mode)
        if traj is None:
            return None
        traj["params"] = _packed_to_training_params(traj["params"])
        return traj
    ...
    def _generate_trajectories_batched(
        ...
        batch = super()._generate_trajectories_batched(keys, n_steps, dt, simulation_mode)
        batch["params"] = jax.vmap(_packed_to_training_params)(batch["params"])
        return batch
