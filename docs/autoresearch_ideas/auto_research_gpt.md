...
These focus on `dte/models/unit/encoder.py`, `dte/models/unit/latent_sde.py`, and `dte/training/shared/losses.py`.
...
1. **Dimensionless / Buckingham-Pi Frontend** (`dte/models/unit/encoder.py`)
2. **Contrastive Cross-System Latent Alignment** (`dte/training/shared/losses.py`)
3. **Koopman + Residual Hybrid Drift** (`dte/models/unit/latent_sde.py`)
4. **Neural CDE Control Integration** (`dte/models/unit/latent_sde.py`)

### Tier 2

5. **Simulator + Residual Dynamics** (`dte/models/unit/latent_sde.py`)
6. **Port-Hamiltonian Dissipative Latent Drift** (`dte/models/unit/latent_sde.py`)
7. **Cross-System Masked Pretraining** (`dte/training/unit/trainer.py`)

### Tier 3

8. **Universal SystemSpec-Conditioned Backbone** (`dte/models/unit/digital_twin.py`)
9. **Compositional Process-Graph Hypernetwork** (`dte/models/unit/digital_twin.py`)
10. **Latent Trajectory Diffusion Foundation Model** (`dte/models/unit/latent_sde.py`)
...
- `configs/autoresearch/autoresearch_gpt_stage1.yaml` (conservative starting point)
- `configs/autoresearch/autoresearch_gpt_stage2.yaml` (more aggressive settings for survivors)
